...
class TestAuthorizationView(_DispatchingViewTestCase):
    ...
    @ddt.data('dop', 'dot')
    def test_post_authorization_view(self, client_type):
        oauth_application = getattr(self, '{}_app'.format(client_type))
        self.client.login(username=self.user.username, password='test')
        allow_field = {'dop': 'authorize', 'dot': 'allow'}
    ...
    def _check_dop_response(self, response):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(self._redirect_destination(response), u'http://testserver/oauth2/authorize/confirm')
        ...
        self.assertEqual(oauth_data['scope'], 6)
